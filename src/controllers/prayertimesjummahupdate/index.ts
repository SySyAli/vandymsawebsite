...
// CronJob for iqamah times

async function refreshIqamahTimes() {
	try {
		const times: PrayerTime[] = await PrayerTimes.find();
		if (times.length === 0) {
			const toBeAdded: PrayerTime = new PrayerTimes({
				fajrIqamah: "N/A",
				sunriseIqamah: "N/A",
				zuhrIqamah: "1:30PM",
				asrIqamah: "5:00pm",
				maghribIqamah: "NA",
				ishaIqamah: "8:30PM",
				firstJummahIqamah: "1:25PM",
				secondJummahIqamah: "2:25PM",
				firstJummahTime: "1:20PM",
				secondJummahTime: "2:20PM",
				maghribDelay: "10",
				jummahUpdate: "Please sign up to do a Khutbah!",
				latitude: 36.14479431053963,
				longitude: -86.80420024114342,
				method: 2,
			});
			await toBeAdded.save();
            console.log("created thing iqamahtimes")
		} else {
            console.log("updated thing iqamahtimes")
            // code for updating (uncomment if any changes are needed)
            /*
			const body = { id: req.params.id, body: req.body };
			const updated: PrayerTime | null = await PrayerTimes.findByIdAndUpdate(
				{ _id: body.id },
				{ $set: body.body }
			);
            */
...
async function refershPrayerTimes() {
	try {
		...
		const data = await axois.get(
			`http://api.aladhan.com/v1/calendar?latitude=${parameters.latitude}&longitude=${parameters.longitude}&method=${parameters.method}&month=${parameters.month}&year=${parameters.year}`
		);
		...
		if (count === 0) {
			...
			await prayertimesapi.findByIdAndUpdate(
				{ _id: oldTimes[0]._id },
				{ everything: currTime }
			);
		}
...
async function addUserTimesAndUpdate(req: Request, res: Response) {
	try {
		...
		res.status(200).json({
			...
			addedTime: newPrayerTime,
			allPrayerTimes: allPrayerTime,
		});
	} catch (err) {
		console.log(err);
		throw err;
...
async function deleteIqamahTime(req: Request, res: Response) {
	try {
		const body = { id: req.params.id, body: req.body };
		const deletedTime: PrayerTime | null = await PrayerTimes.findByIdAndDelete(
			body.id,
			body.body
		);
		const allTimes: PrayerTime[] = await PrayerTimes.find();
...
async function getTodayPrayerTime(req: Request, res: Response) {
	try {
		...
		const prayerTimes24hrs = {
			fajr: removeCST(currTime.timings.Fajr) + ":00",
        ...
        */
		...
		const prayerTimes12hrs = {
			fajr: convertTo12HRTime(prayerTimes24hrs.fajr),
		...
		res.status(201).json({
			maghribiqammah: addingMinutes(
				removeCST(prayerTimes24hrs.maghrib) + ":00",
				Number(parameters.maghribDelay)
			),
			prayerTimes: {
				fajr: prayerTimes12hrs.fajr,
				sunrise: prayerTimes12hrs.sunrise,
				zuhr: prayerTimes12hrs.zuhr,
				asr: prayerTimes12hrs.asr,
				maghrib: prayerTimes12hrs.maghrib,
				isha: prayerTimes12hrs.isha,
			},
			IslamicDate: {
				day: currTime.date.hijri.day,
				month: changeMonthName(currTime.date.hijri.month.number),
				year: currTime.date.hijri.year,
			},
			timezone: currTime.meta.timezone,
			currentTimeStamp:
				convertTo12HRTime(currentTimeStamp) +
				", " +
				(date.getMonth() + 1) +
				"/" +
				date.getDate() +
...
function findCurrentSalah(currentTime: string, prayerTimes: any) {
	...
	if (
		newTime < new Date("1970-01-01T" + prayerTimes.fajr) ||
		newTime > new Date("1970-01-01T" + prayerTimes.isha)
	) {
		return "isha";
...
export {
	...
	//updateIqamahTime,
	//deleteIqamahTime,
	getTodayPrayerTime, refreshIqamahTimes
};
